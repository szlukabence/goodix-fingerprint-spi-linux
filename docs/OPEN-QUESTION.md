...
| Wrong clock mode or speed | Brute-forced: modes 0–3 × 10 MHz, 1 MHz, 400 kHz, 100 kHz, **50 kHz**, flagging *any* byte that is not `0x00`/`0xff`. All returned `ff`. **Both axes independently verified to have actually reached the hardware**: speeds timed against theory (50 kHz measured 164.3 ms vs 163.8 ms theoretical, within 0.3%), and clock mode read back from the controller's `SSCR1` (`0x1dc0`/`0x1dd0`/`0x1dc8`/`0x1dd8` — SPO/SPH correct for all four). 50 kHz is 200× below rated speed, so signal integrity would have worked there. (LSB-first is rejected by `pxa2xx-spi`.) |
...
| Settle time, word delay, word size, header/body gap, read length | Swept: settle after reset 0.15–**5 s**, inter-byte delay 0–100 µs, 8/16 bits per word, header→body gap 0–100 ms, read windows to 8 KiB, and 200× repeated hammering. All `ff`. (Note: a single spidev transfer cannot exceed `bufsiz`, 4096 by default — chunk larger reads; 32 bits/word is rejected by `pxa2xx-spi`.) |
| The TPM / Windows Hello credential path | Not involved. The TPM protects the credential released *after* a match; it never talks to the sensor. Matching runs on the host CPU — this part has no match-on-chip command. `gfspi.dll` imports only `sgx_urts.dll`/`sgx_uae_service.dll`; no TPM API at all. |
| The power-button integration | `PWRB` (`PNP0C0C`) has no `_CRS` whatsoever — no GPIO, no interrupt, no resources. The sensor's pads appear in exactly 8 places in ~52,000 lines of ACPI, all inside its own `_INI`/`_CRS`. Zero shared resources — and **both working siblings are power-button integrated too**, so it cannot be the differentiator. |
| Speaking the wrong protocol entirely | Swept **the whole Milan command space** — cmd0 `0x0`–`0xF` × cmd1 `0`–`7`, 128 correctly-framed commands — plus every other Goodix dialect anyone has documented: OpenGoodixSPI's bare `0xB0`/`0xF0`/`0x0C` patterns with no transport header, GR5515 DFU framing (magic `GD`), the `0x00010000` IHA framing, the `0xccf2` magic, and bare `00`/`ff`/`aa`/`55`/ramp patterns. All `ff`. See `tools/dialects.py`. |
| The full command space behind a real power cycle | Re-ran all 128 Milan commands (cmd0 `0x0`–`0xF` × cmd1 `0`–`7`) after a genuine 500 ms power cycle — 5.6× the measured 88.8 ms rail decay — rather than the 10 ms no-op pulse the original sweep used. Every one still `ff`. |
